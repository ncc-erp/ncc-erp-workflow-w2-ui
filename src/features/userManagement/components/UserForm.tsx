import { Button, Checkbox, Divider, Stack, Tab, TabList, TabPanel, TabPanels, Tabs } from "@chakra-ui/react";
...
const UserForm = ({ initialValues, userId, onClose }: UserFormProps) => {
    ...
    const { mutate, isLoading, isSuccess, isError } = useUpdateUser(userId, userValues);
    ...
    const handleUpdateResult = () => {
        queryClient.invalidateQueries(['filterUser']);
        queryClient.invalidateQueries(['getRoleByUserId', userId]);
        onClose();
        if (isSuccess) {
    ...
    const handleChangeRolesCheckbox = (roleName: string, checked: boolean) => {
        if (checked) {
            ...
            const updatedRoles = formik.values.roleNames.filter((role) => role !== roleName);
                        ...
                        <TabPanel>
                            ...
                            <Stack mt={5} direction='column'>
                                <Checkbox
                                    ...
                                    onChange={(e) => handleChangeCheckbox('isActive', e.target.checked)}
                                >
                                    Active
                                </Checkbox>
                                <Checkbox
                                    ...
                                    onChange={(e) => handleChangeCheckbox('lockoutEnabled', e.target.checked)}
                                >
                        ...
                        <TabPanel>
                            <Stack mt={5} direction='column'>
                                <Checkbox
                                    ...
                                    onChange={(e) => handleChangeRolesCheckbox(UserRoles.ADMIN, e.target.checked)}
                                >
                                    {UserRoles.ADMIN}
                                </Checkbox>
                                <Checkbox
                                    colorScheme='blue'
                                    isChecked={formik.values.roleNames.includes(UserRoles.DEFAULT_USER)}
                                    onChange={(e) => handleChangeRolesCheckbox(UserRoles.DEFAULT_USER, e.target.checked)}
                                >
